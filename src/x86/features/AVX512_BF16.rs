...
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VCVTNE2PS2BF16.html).
///
...
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VCVTNEPS2BF16.html).
///
/// Supported operand variants:
///
/// ```text
...
impl<'a> Vcvtneps2bf16Emitter<Xmm, Xmm> for Assembler<'a> {
    fn vcvtneps2bf16(&mut self, op0: Xmm, op1: Xmm) {
        self.emit(VCVTNEPS2BF16_128RR, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16Emitter<Xmm, Mem> for Assembler<'a> {
    fn vcvtneps2bf16(&mut self, op0: Xmm, op1: Mem) {
        self.emit(VCVTNEPS2BF16_128RM, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16Emitter<Xmm, Ymm> for Assembler<'a> {
    fn vcvtneps2bf16(&mut self, op0: Xmm, op1: Ymm) {
        self.emit(VCVTNEPS2BF16_256RR, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16Emitter<Ymm, Zmm> for Assembler<'a> {
    fn vcvtneps2bf16(&mut self, op0: Ymm, op1: Zmm) {
        self.emit(VCVTNEPS2BF16_512RR, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16Emitter<Ymm, Mem> for Assembler<'a> {
    fn vcvtneps2bf16(&mut self, op0: Ymm, op1: Mem) {
        self.emit(VCVTNEPS2BF16_512RM, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

/// `VCVTNEPS2BF16_MASK` (VCVTNEPS2BF16). 
/// Converts one SIMD register of packed single data into a single register of packed BF16 data.
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VCVTNEPS2BF16.html).
...
impl<'a> Vcvtneps2bf16MaskEmitter<Xmm, Xmm> for Assembler<'a> {
    fn vcvtneps2bf16_mask(&mut self, op0: Xmm, op1: Xmm) {
        self.emit(VCVTNEPS2BF16_128RR_MASK, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16MaskEmitter<Xmm, Mem> for Assembler<'a> {
    fn vcvtneps2bf16_mask(&mut self, op0: Xmm, op1: Mem) {
        self.emit(VCVTNEPS2BF16_128RM_MASK, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16MaskEmitter<Xmm, Ymm> for Assembler<'a> {
    fn vcvtneps2bf16_mask(&mut self, op0: Xmm, op1: Ymm) {
        self.emit(VCVTNEPS2BF16_256RR_MASK, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16MaskEmitter<Ymm, Zmm> for Assembler<'a> {
    fn vcvtneps2bf16_mask(&mut self, op0: Ymm, op1: Zmm) {
        self.emit(VCVTNEPS2BF16_512RR_MASK, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> Vcvtneps2bf16MaskEmitter<Ymm, Mem> for Assembler<'a> {
    fn vcvtneps2bf16_mask(&mut self, op0: Ymm, op1: Mem) {
        self.emit(VCVTNEPS2BF16_512RM_MASK, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

/// `VCVTNEPS2BF16_MASKZ` (VCVTNEPS2BF16). 
/// Converts one SIMD register of packed single data into a single register of packed BF16 data.
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VCVTNEPS2BF16.html).
...
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VDPBF16PS.html).
///
/// Supported operand variants:
///
/// ```text
...
impl<'a> Vdpbf16psEmitter<Xmm, Xmm, Xmm> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Xmm, op1: Xmm, op2: Xmm) {
        self.emit(VDPBF16PS128RRR, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

impl<'a> Vdpbf16psEmitter<Xmm, Xmm, Mem> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Xmm, op1: Xmm, op2: Mem) {
        self.emit(VDPBF16PS128RRM, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

impl<'a> Vdpbf16psEmitter<Ymm, Ymm, Ymm> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Ymm, op1: Ymm, op2: Ymm) {
        self.emit(VDPBF16PS256RRR, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

impl<'a> Vdpbf16psEmitter<Ymm, Ymm, Mem> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Ymm, op1: Ymm, op2: Mem) {
        self.emit(VDPBF16PS256RRM, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

impl<'a> Vdpbf16psEmitter<Zmm, Zmm, Zmm> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Zmm, op1: Zmm, op2: Zmm) {
        self.emit(VDPBF16PS512RRR, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

impl<'a> Vdpbf16psEmitter<Zmm, Zmm, Mem> for Assembler<'a> {
    fn vdpbf16ps(&mut self, op0: Zmm, op1: Zmm, op2: Mem) {
        self.emit(VDPBF16PS512RRM, op0.as_operand(), op1.as_operand(), op2.as_operand(), &NOREG);
    }
}

/// `VDPBF16PS_MASK` (VDPBF16PS). 
/// This instruction performs a SIMD dot-product of two BF16 pairs and accumulates into a packed single precision register.
///
///
/// For more details, see the [Intel manual](https://www.felixcloutier.com/x86/VDPBF16PS.html).
...
impl<'a> Assembler<'a> {
    ...
    where Assembler<'a>: Vcvtne2ps2bf16Emitter<A, B, C> {
    ...
    where Assembler<'a>: Vcvtne2ps2bf16MaskEmitter<A, B, C> {
    ...
    where Assembler<'a>: Vcvtne2ps2bf16MaskzEmitter<A, B, C> {
    ...
    where Assembler<'a>: Vcvtneps2bf16Emitter<A, B> {
    ...
    where Assembler<'a>: Vcvtneps2bf16MaskEmitter<A, B> {
    ...
    where Assembler<'a>: Vcvtneps2bf16MaskzEmitter<A, B> {
    ...
    where Assembler<'a>: Vdpbf16psEmitter<A, B, C> {
    ...
    where Assembler<'a>: Vdpbf16psMaskEmitter<A, B, C> {
    ...
    where Assembler<'a>: Vdpbf16psMaskzEmitter<A, B, C> {
