pub trait X86GFNIEmitter: Emitter {
    ...
    fn gf2p8affineinvqb(&mut self,op0: impl OperandCast,op1: impl OperandCast,op2: impl OperandCast) -> () {
        ...
        if op0.is_vec() && op1.is_vec() && op2.is_imm() {
            ...
            unreachable!("invalid operand types for GF2P8AFFINEINVQB");
        }
    }
    /// Emits `GF2P8AFFINEQB`.
    fn gf2p8affineqb(&mut self,op0: impl OperandCast,op1: impl OperandCast,op2: impl OperandCast) -> () {
        let op0 = op0.as_operand();
        let op1 = op1.as_operand();
        let op2 = op2.as_operand();
        if op0.is_vec() && op1.is_vec() && op2.is_imm() {
            ...
            unreachable!("invalid operand types for GF2P8AFFINEQB");
        }
    }
    /// Emits `GF2P8MULB`.
    fn gf2p8mulb(&mut self,op0: impl OperandCast,op1: impl OperandCast) -> () {
        let op0 = op0.as_operand();
        let op1 = op1.as_operand();
        if op0.is_vec() && op1.is_vec() {
            ...
            unreachable!("invalid operand types for GF2P8MULB");
        }
    }
}
