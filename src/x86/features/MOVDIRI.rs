pub trait X86MOVDIRIEmitter: Emitter {
    /// Emits `MOVDIRI32MR`.
    fn movdiri32(&mut self,op0: impl OperandCast,op1: impl OperandCast) -> Result<(), AsmError> {
        self.emit(MOVDIRI32MR, op0.as_operand(),op1.as_operand(),&NOREG,&NOREG);
        if let Some(err) = self.last_error() { Err(err) } else { Ok(()) }
    }
    /// Emits `MOVDIRI64MR`.
    fn movdiri64(&mut self,op0: impl OperandCast,op1: impl OperandCast) -> Result<(), AsmError> {
