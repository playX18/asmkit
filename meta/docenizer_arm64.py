...
DEFAULT_DOCUMENTATION_URL = 'https://developer.arm.com/documentation/'
...
def resolve_instruction_directory(inputfolder):
    candidates = [inputfolder, os.path.join(inputfolder, 'xml')]
    for candidate in candidates:
        ...
        try:
            ...
            continue
        if any(entry.endswith('.xml') for entry in entries):
            return candidate
    raise FileNotFoundError(f"Unable to locate ARM XML docs under {inputfolder}")


def collect_instruction_docs(inputfolder, documentation_url=None):
    """Return uppercase instruction-name -> metadata (tooltip/url/html)."""
    directory = resolve_instruction_directory(inputfolder)
    instructions = parse_xml(directory)
    docs = {}
    doc_url = documentation_url or DEFAULT_DOCUMENTATION_URL
