pub trait X86SSE4AEmitter: Emitter {
    /// Emits `SSE_EXTRQ`.
    fn sse_extrq(&mut self,op0: impl OperandCast,op1: impl OperandCast) -> () {
        ...
        if op0.is_vec() && op1.is_imm() {
            ...
            unreachable!("invalid operand types for SSE_EXTRQ");
        }
    }
    /// Emits `SSE_INSERTQRR`.
    fn sse_insertq(&mut self,op0: impl OperandCast,op1: impl OperandCast) -> () {
        self.emit(SSE_INSERTQRR, op0.as_operand(),op1.as_operand(),&NOREG,&NOREG);
    }
    /// Emits `SSE_INSERTQRRI`.
    fn sse_insertq_3(&mut self,op0: impl OperandCast,op1: impl OperandCast,op2: impl OperandCast) -> () {
