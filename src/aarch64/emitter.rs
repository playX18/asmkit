...
pub trait A64EmitterExplicit: Emitter {
    fn udf(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::UDF as _, &[imm16.as_operand()])
    }
    ...
    fn ngcw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NGCw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn ngcsw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NGCSw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn ngcx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NGCx as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn ngcsx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NGCSx as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn addw_uxtb(
        &mut self,
        rd: impl OperandCast,
    ...
    fn cmnwi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::CMNwi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn cmpwi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::CMPwi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn cmnxi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::CMNxi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn cmpxi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::CMPxi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn mov_spw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MOV_SPw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn mov_spx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MOV_SPx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addw_lsl(
        &mut self,
        rd: impl OperandCast,
    ...
    fn cmnw(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::CMNw as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn cmpw(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::CMPw as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn cmnx(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::CMNx as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn cmpx(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::CMPx as _, &[rn.as_operand(), rm.as_operand()])
    }
    ...
    fn negw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NEGw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn negsw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NEGSw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn negx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NEGx as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn negsx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::NEGSx as _, &[rd.as_operand(), rm.as_operand()])
    }
    ...
    fn tstwi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::TSTwi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn tstxi(&mut self, rn: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::TSTxi as _, &[rn.as_operand(), imm.as_operand()])
    }
    fn andw_lsl(
        &mut self,
        rd: impl OperandCast,
    ...
    fn tstw(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::TSTw as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn tstx(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::TSTx as _, &[rn.as_operand(), rm.as_operand()])
    }
    ...
    fn mvnw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::MVNw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn mvnx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::MVNx as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn movw(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::MOVw as _, &[rd.as_operand(), rm.as_operand()])
    }
    fn movx(&mut self, rd: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::MOVx as _, &[rd.as_operand(), rm.as_operand()])
    }
    ...
    fn b(&mut self, imm26: impl OperandCast) {
        self.emit_n(Opcode::B as _, &[imm26.as_operand()])
    }
    fn bl(&mut self, imm26: impl OperandCast) {
        self.emit_n(Opcode::BL as _, &[imm26.as_operand()])
    }
    fn sbfmw(
        &mut self,
        rd: impl OperandCast,
    ...
    fn sxtbw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTBw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtbw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTBw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtbx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTBx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxthw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTHw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxthw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTHw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxthx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTHx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtwx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTWx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn br(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BR as _, &[rn.as_operand()])
    }
    fn braaz(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BRAAZ as _, &[rn.as_operand()])
    }
    fn brabz(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BRABZ as _, &[rn.as_operand()])
    }
    fn blr(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BLR as _, &[rn.as_operand()])
    }
    fn blraaz(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BLRAAZ as _, &[rn.as_operand()])
    }
    fn blrabz(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::BLRABZ as _, &[rn.as_operand()])
    }
    fn ret(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::RET as _, &[rn.as_operand()])
    }
    fn retaa(&mut self) {
        self.emit_n(Opcode::RETAA as _, &[])
    }
    fn retab(&mut self) {
        self.emit_n(Opcode::RETAB as _, &[])
    }
    fn braa(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::BRAA as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn brab(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::BRAB as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn blraa(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::BLRAA as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn blrab(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::BLRAB as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn brk(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::BRK as _, &[imm16.as_operand()])
    }
    fn cbzw(&mut self, rt: impl OperandCast, imm19: impl OperandCast) {
        self.emit_n(Opcode::CBZw as _, &[rt.as_operand(), imm19.as_operand()])
    }
    fn cbnzw(&mut self, rt: impl OperandCast, imm19: impl OperandCast) {
        self.emit_n(Opcode::CBNZw as _, &[rt.as_operand(), imm19.as_operand()])
    }
    fn cbzx(&mut self, rt: impl OperandCast, imm19: impl OperandCast) {
        self.emit_n(Opcode::CBZx as _, &[rt.as_operand(), imm19.as_operand()])
    }
    fn cbnzx(&mut self, rt: impl OperandCast, imm19: impl OperandCast) {
        self.emit_n(Opcode::CBNZx as _, &[rt.as_operand(), imm19.as_operand()])
    }
    ...
    fn clrex(&mut self) {
        self.emit_n(Opcode::CLREX as _, &[])
    }
    fn dsb(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::DSB as _, &[crm.as_operand()])
    }
    fn dmb(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::DMB as _, &[crm.as_operand()])
    }
    fn isb(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::ISB as _, &[crm.as_operand()])
    }
    fn ssbb(&mut self) {
        self.emit_n(Opcode::SSBB as _, &[])
    }
    fn hint(&mut self, imm: impl OperandCast) {
        self.emit_n(Opcode::HINT as _, &[imm.as_operand()])
    }
    fn nop(&mut self) {
        self.emit_n(Opcode::NOP as _, &[])
    }
    fn r#yield(&mut self) {
        self.emit_n(Opcode::YIELD as _, &[])
    }
    fn wfe(&mut self) {
        self.emit_n(Opcode::WFE as _, &[])
    }
    fn wfi(&mut self) {
        self.emit_n(Opcode::WFI as _, &[])
    }
    fn sev(&mut self) {
        self.emit_n(Opcode::SEV as _, &[])
    }
    fn sevl(&mut self) {
        self.emit_n(Opcode::SEVL as _, &[])
    }
    fn dgh(&mut self) {
        self.emit_n(Opcode::DGH as _, &[])
    }
    fn xpaclri(&mut self) {
        self.emit_n(Opcode::XPACLRI as _, &[])
    }
    fn pacia1716(&mut self) {
        self.emit_n(Opcode::PACIA1716 as _, &[])
    }
    fn pacib1716(&mut self) {
        self.emit_n(Opcode::PACIB1716 as _, &[])
    }
    fn autia1716(&mut self) {
        self.emit_n(Opcode::AUTIA1716 as _, &[])
    }
    fn autib1716(&mut self) {
        self.emit_n(Opcode::AUTIB1716 as _, &[])
    }
    fn esb(&mut self) {
        self.emit_n(Opcode::ESB as _, &[])
    }
    fn csdb(&mut self) {
        self.emit_n(Opcode::CSDB as _, &[])
    }
    fn clrbhb(&mut self) {
        self.emit_n(Opcode::CLRBHB as _, &[])
    }
    fn paciaz(&mut self) {
        self.emit_n(Opcode::PACIAZ as _, &[])
    }
    fn paciasp(&mut self) {
        self.emit_n(Opcode::PACIASP as _, &[])
    }
    fn pacibz(&mut self) {
        self.emit_n(Opcode::PACIBZ as _, &[])
    }
    fn pacibsp(&mut self) {
        self.emit_n(Opcode::PACIBSP as _, &[])
    }
    fn autiaz(&mut self) {
        self.emit_n(Opcode::AUTIAZ as _, &[])
    }
    fn autiasp(&mut self) {
        self.emit_n(Opcode::AUTIASP as _, &[])
    }
    fn autibz(&mut self) {
        self.emit_n(Opcode::AUTIBZ as _, &[])
    }
    fn autibsp(&mut self) {
        self.emit_n(Opcode::AUTIBSP as _, &[])
    }
    fn bti(&mut self) {
        self.emit_n(Opcode::BTI as _, &[])
    }
    fn btic(&mut self) {
        self.emit_n(Opcode::BTIc as _, &[])
    }
    fn btij(&mut self) {
        self.emit_n(Opcode::BTIj as _, &[])
    }
    fn btijc(&mut self) {
        self.emit_n(Opcode::BTIjc as _, &[])
    }
    fn chkfeat(&mut self) {
        self.emit_n(Opcode::CHKFEAT as _, &[])
    }
    fn hlt(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::HLT as _, &[imm16.as_operand()])
    }
    fn svc(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::SVC as _, &[imm16.as_operand()])
    }
    fn hvc(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::HVC as _, &[imm16.as_operand()])
    }
    fn smc(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::SMC as _, &[imm16.as_operand()])
    }
    fn dcps1(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::DCPS1 as _, &[imm16.as_operand()])
    }
    fn dcps2(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::DCPS2 as _, &[imm16.as_operand()])
    }
    fn dcps3(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::DCPS3 as _, &[imm16.as_operand()])
    }
    fn eret(&mut self) {
        self.emit_n(Opcode::ERET as _, &[])
    }
    fn eretaa(&mut self) {
        self.emit_n(Opcode::ERETAA as _, &[])
    }
    fn eretab(&mut self) {
        self.emit_n(Opcode::ERETAB as _, &[])
    }
    fn drps(&mut self) {
        self.emit_n(Opcode::DRPS as _, &[])
    }
    fn clzw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clsw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLSw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ctzw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CTZw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cntw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CNTw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn absw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABSw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clzx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clsx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLSx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ctzx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CTZx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cntx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CNTx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn absx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABSx as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn csetw(&mut self, rd: impl OperandCast, cond: impl OperandCast) {
        self.emit_n(Opcode::CSETw as _, &[rd.as_operand(), cond.as_operand()])
    }
    fn csetmw(&mut self, rd: impl OperandCast, cond: impl OperandCast) {
        self.emit_n(Opcode::CSETMw as _, &[rd.as_operand(), cond.as_operand()])
    }
    fn csetx(&mut self, rd: impl OperandCast, cond: impl OperandCast) {
        self.emit_n(Opcode::CSETx as _, &[rd.as_operand(), cond.as_operand()])
    }
    fn csetmx(&mut self, rd: impl OperandCast, cond: impl OperandCast) {
        self.emit_n(Opcode::CSETMx as _, &[rd.as_operand(), cond.as_operand()])
    }
    fn extrw(
        &mut self,
        rd: impl OperandCast,
    ...
    fn movnw(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNw as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movnw16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNw16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzw(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZw as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzw16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZw16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkw(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKw as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkw16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKw16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movnx(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNx as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movnx16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNx16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movnx32(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNx32 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movnx48(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVNx48 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzx(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZx as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzx16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZx16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzx32(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZx32 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movzx48(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVZx48 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkx(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKx as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkx16(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKx16 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkx32(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKx32 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    fn movkx48(&mut self, rd: impl OperandCast, imm16: impl OperandCast) {
        self.emit_n(Opcode::MOVKx48 as _, &[rd.as_operand(), imm16.as_operand()])
    }
    ...
    fn sys(&mut self, sysreg: impl OperandCast, rt: impl OperandCast) {
        self.emit_n(Opcode::SYS as _, &[sysreg.as_operand(), rt.as_operand()])
    }
    fn sysl(&mut self, rt: impl OperandCast, sysreg: impl OperandCast) {
        self.emit_n(Opcode::SYSL as _, &[rt.as_operand(), sysreg.as_operand()])
    }
    fn at_s1e1r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E1R as _, &[rt.as_operand()])
    }
    fn at_s1e1w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E1W as _, &[rt.as_operand()])
    }
    fn at_s1e0r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E0R as _, &[rt.as_operand()])
    }
    fn at_s1e0w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E0W as _, &[rt.as_operand()])
    }
    fn at_s1e1rp(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E1RP as _, &[rt.as_operand()])
    }
    fn at_s1e1wp(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E1WP as _, &[rt.as_operand()])
    }
    fn at_s1e2r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E2R as _, &[rt.as_operand()])
    }
    fn at_s1e2w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E2W as _, &[rt.as_operand()])
    }
    fn at_s12e1r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S12E1R as _, &[rt.as_operand()])
    }
    fn at_s12e1w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S12E1W as _, &[rt.as_operand()])
    }
    fn at_s12e0r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S12E0R as _, &[rt.as_operand()])
    }
    fn at_s12e0w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S12E0W as _, &[rt.as_operand()])
    }
    fn at_s1e3r(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E3R as _, &[rt.as_operand()])
    }
    fn at_s1e3w(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::AT_S1E3W as _, &[rt.as_operand()])
    }
    fn dc_ivac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_IVAC as _, &[rt.as_operand()])
    }
    fn dc_isw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_ISW as _, &[rt.as_operand()])
    }
    fn dc_igvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_IGVAC as _, &[rt.as_operand()])
    }
    fn dc_igsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_IGSW as _, &[rt.as_operand()])
    }
    fn dc_igdvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_IGDVAC as _, &[rt.as_operand()])
    }
    fn dc_igdsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_IGDSW as _, &[rt.as_operand()])
    }
    fn dc_csw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CSW as _, &[rt.as_operand()])
    }
    fn dc_cgsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGSW as _, &[rt.as_operand()])
    }
    fn dc_cgdsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGDSW as _, &[rt.as_operand()])
    }
    fn dc_cisw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CISW as _, &[rt.as_operand()])
    }
    fn dc_cigsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CIGSW as _, &[rt.as_operand()])
    }
    fn dc_cigdsw(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CIGDSW as _, &[rt.as_operand()])
    }
    fn dc_zva(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_ZVA as _, &[rt.as_operand()])
    }
    fn dc_gva(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_GVA as _, &[rt.as_operand()])
    }
    fn dc_gzva(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_GZVA as _, &[rt.as_operand()])
    }
    fn dc_cvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CVAC as _, &[rt.as_operand()])
    }
    fn dc_cgvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGVAC as _, &[rt.as_operand()])
    }
    fn dc_cgdvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGDVAC as _, &[rt.as_operand()])
    }
    fn dc_cvau(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CVAU as _, &[rt.as_operand()])
    }
    fn dc_cvap(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CVAP as _, &[rt.as_operand()])
    }
    fn dc_cgvap(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGVAP as _, &[rt.as_operand()])
    }
    fn dc_cgdvap(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGDVAP as _, &[rt.as_operand()])
    }
    fn dc_cvadp(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CVADP as _, &[rt.as_operand()])
    }
    fn dc_cgvadp(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGVADP as _, &[rt.as_operand()])
    }
    fn dc_cgdvadp(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CGDVADP as _, &[rt.as_operand()])
    }
    fn dc_civac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CIVAC as _, &[rt.as_operand()])
    }
    fn dc_cigvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CIGVAC as _, &[rt.as_operand()])
    }
    fn dc_cigdvac(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::DC_CIGDVAC as _, &[rt.as_operand()])
    }
    fn ic_ialluis(&mut self) {
        self.emit_n(Opcode::IC_IALLUIS as _, &[])
    }
    fn ic_iallu(&mut self) {
        self.emit_n(Opcode::IC_IALLU as _, &[])
    }
    fn ic_ivau(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::IC_IVAU as _, &[rt.as_operand()])
    }
    fn tlbi_vmalle1is(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLE1IS as _, &[])
    }
    fn tlbi_vae1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_aside1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_ASIDE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_vaae1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAAE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_vale1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_vaale1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAALE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_vmalle1(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLE1 as _, &[])
    }
    fn tlbi_vae1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE1 as _, &[rt.as_operand()])
    }
    fn tlbi_aside1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_ASIDE1 as _, &[rt.as_operand()])
    }
    fn tlbi_vaae1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAAE1 as _, &[rt.as_operand()])
    }
    fn tlbi_vale1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE1 as _, &[rt.as_operand()])
    }
    fn tlbi_vaale1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAALE1 as _, &[rt.as_operand()])
    }
    fn tlbi_ipas2e1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2E1IS as _, &[rt.as_operand()])
    }
    fn tlbi_ipas2le1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2LE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_alle2is(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE2IS as _, &[])
    }
    fn tlbi_vae2is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE2IS as _, &[rt.as_operand()])
    }
    fn tlbi_alle1is(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE1IS as _, &[])
    }
    fn tlbi_vale2is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE2IS as _, &[rt.as_operand()])
    }
    fn tlbi_vmalls12e1is(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLS12E1IS as _, &[])
    }
    fn tlbi_ipas2e1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2E1 as _, &[rt.as_operand()])
    }
    fn tlbi_ipas2le1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2LE1 as _, &[rt.as_operand()])
    }
    fn tlbi_alle2(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE2 as _, &[])
    }
    fn tlbi_vae2(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE2 as _, &[rt.as_operand()])
    }
    fn tlbi_alle1(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE1 as _, &[])
    }
    fn tlbi_vale2(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE2 as _, &[rt.as_operand()])
    }
    fn tlbi_vmalls12e1(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLS12E1 as _, &[])
    }
    fn tlbi_alle3is(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE3IS as _, &[])
    }
    fn tlbi_vae3is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE3IS as _, &[rt.as_operand()])
    }
    fn tlbi_vale3is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE3IS as _, &[rt.as_operand()])
    }
    fn tlbi_alle3(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE3 as _, &[])
    }
    fn tlbi_vae3(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE3 as _, &[rt.as_operand()])
    }
    fn tlbi_vale3(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE3 as _, &[rt.as_operand()])
    }
    fn tlbi_vmalle1os(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLE1OS as _, &[])
    }
    fn tlbi_vae1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_aside1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_ASIDE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_vaae1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAAE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_vale1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_vaale1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAALE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_alle2os(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE2OS as _, &[])
    }
    fn tlbi_vae2os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE2OS as _, &[rt.as_operand()])
    }
    fn tlbi_alle1os(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE1OS as _, &[])
    }
    fn tlbi_vale2os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE2OS as _, &[rt.as_operand()])
    }
    fn tlbi_vmalls12e1os(&mut self) {
        self.emit_n(Opcode::TLBI_VMALLS12E1OS as _, &[])
    }
    fn tlbi_ipas2e1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2E1OS as _, &[rt.as_operand()])
    }
    fn tlbi_ipas2le1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_IPAS2LE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_alle3os(&mut self) {
        self.emit_n(Opcode::TLBI_ALLE3OS as _, &[])
    }
    fn tlbi_vae3os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VAE3OS as _, &[rt.as_operand()])
    }
    fn tlbi_vale3os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_VALE3OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvaae1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAAE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvaale1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAALE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvaae1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAAE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvaale1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAALE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE1 as _, &[rt.as_operand()])
    }
    fn tlbi_rvaae1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAAE1 as _, &[rt.as_operand()])
    }
    fn tlbi_rvale1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE1 as _, &[rt.as_operand()])
    }
    fn tlbi_rvaale1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAALE1 as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2e1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2E1IS as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2le1is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2LE1IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae2is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE2IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale2is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE2IS as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2e1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2E1 as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2e1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2E1OS as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2le1(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2LE1 as _, &[rt.as_operand()])
    }
    fn tlbi_ripas2le1os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RIPAS2LE1OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae2os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE2OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale2os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE2OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae2(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE2 as _, &[rt.as_operand()])
    }
    fn tlbi_rvale2(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE2 as _, &[rt.as_operand()])
    }
    fn tlbi_rvae3is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE3IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale3is(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE3IS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae3os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE3OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvale3os(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE3OS as _, &[rt.as_operand()])
    }
    fn tlbi_rvae3(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVAE3 as _, &[rt.as_operand()])
    }
    fn tlbi_rvale3(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TLBI_RVALE3 as _, &[rt.as_operand()])
    }
    fn msr(&mut self, sysreg: impl OperandCast, rt: impl OperandCast) {
        self.emit_n(Opcode::MSR as _, &[sysreg.as_operand(), rt.as_operand()])
    }
    fn mrs(&mut self, rt: impl OperandCast, sysreg: impl OperandCast) {
        self.emit_n(Opcode::MRS as _, &[rt.as_operand(), sysreg.as_operand()])
    }
    ...
    fn msri_uao(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_UAO as _, &[crm.as_operand()])
    }
    fn msri_pan(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_PAN as _, &[crm.as_operand()])
    }
    fn msri_spsel(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_SPSel as _, &[crm.as_operand()])
    }
    fn msri_ssbs(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_SSBS as _, &[crm.as_operand()])
    }
    fn msri_dit(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_DIT as _, &[crm.as_operand()])
    }
    fn msri_tco(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_TCO as _, &[crm.as_operand()])
    }
    fn msri_daifset(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_DAIFSet as _, &[crm.as_operand()])
    }
    fn msri_daifclr(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_DAIFClr as _, &[crm.as_operand()])
    }
    fn msri_allint(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_ALLINT as _, &[crm.as_operand()])
    }
    fn msri_pm(&mut self, crm: impl OperandCast) {
        self.emit_n(Opcode::MSRi_PM as _, &[crm.as_operand()])
    }
    fn rbitw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::RBITw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev16w(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV16w as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32w(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32w as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rbitx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::RBITx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev16x(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV16x as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32x(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32x as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64x(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64x as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn stllrb(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLLRB as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stlrb(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLRB as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldlarb(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDLARB as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldarb(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDARB as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stllrh(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLLRH as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stlrh(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLRH as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldlarh(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDLARH as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldarh(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDARH as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stllrw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLLRw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stlrw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLRw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldlarw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDLARw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldarw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDARw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stllrx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLLRx as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stlrx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STLRx as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldlarx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDLARx as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldarx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDARx as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn ldxrbw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDXRBw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaxrbw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAXRBw as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn ldxrhw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDXRHw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaxrhw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAXRHw as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn ldxrw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDXRw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaxrw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAXRw as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn ldxrx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDXRx as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaxrx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAXRx as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn st4_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st4_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST4_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_4_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_4_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st3_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST3_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_3_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_3_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_1_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_1_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st2_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST2_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn st1_2_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST1_2_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_4_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_4_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_3_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_3_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_1_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_1_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1_2_2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1_2_2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn ld1r8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r8b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R8b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r4h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R4h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r2s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R2s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r1d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R1d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld1r2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD1R2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld3r2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD3R2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld2r2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD2R2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r16b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R16b as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r8h(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R8h as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r4s(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R4s as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld4r2d(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD4R2d as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn fcvtnsws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtasws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpsws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuwd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtaswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauwd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuwd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuwd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuwd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnswh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtaswh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpswh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmswh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzswh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnsxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtasxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpsxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUxs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnsxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtasxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpsxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnsxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtasxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpsxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpuxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfsw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFsw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfsw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFsw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfdw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFdw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfdw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFdw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfhw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFhw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfhw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFhw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfsx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFsx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfsx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFsx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfdx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFdx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfdx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFdx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfhx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFhx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfhx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFhx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovws(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVws as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovwh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVwh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVxd as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fmovxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVxh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovsw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVsw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovhw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVhw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovdx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVdx as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fmovhx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVhx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fjcvtzswd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FJCVTZSwd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtds(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTds as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvths(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVThs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTsd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn bfcvt(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::BFCVT as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvthd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVThd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTsh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtdh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTdh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintns(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTNs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintms(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTMs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintzs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintas(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTAs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTXs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintis(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTIs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintnd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTNd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintmd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTMd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintzd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintad(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTAd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTXd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintid(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTId as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintnh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTNh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintmh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTMh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintzh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintah(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTAh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTXh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintih(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTIh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32zs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Zs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32xs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Xs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64zs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Zs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64xs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Xs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32zd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Zd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32xd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Xd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64zd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Zd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64xd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Xd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fabss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fnegs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEGs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrts(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRTs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fabsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fnegd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEGd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrtd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRTd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMOVh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fabsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fnegh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEGh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrth(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRTh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmovsi(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOVsi as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmovdi(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOVdi as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmovhi(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOVhi as _, &[rd.as_operand(), imm.as_operand()])
    }
    ...
    fn fcmp_s(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMP_s as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmp_0s(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMP_0s as _, &[rn.as_operand()])
    }
    fn fcmpe_s(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_s as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmpe_0s(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_0s as _, &[rn.as_operand()])
    }
    fn fcmp_d(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMP_d as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmp_0d(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMP_0d as _, &[rn.as_operand()])
    }
    fn fcmpe_d(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_d as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmpe_0d(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_0d as _, &[rn.as_operand()])
    }
    fn fcmp_h(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMP_h as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmp_0h(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMP_0h as _, &[rn.as_operand()])
    }
    fn fcmpe_h(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_h as _, &[rn.as_operand(), rm.as_operand()])
    }
    fn fcmpe_0h(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::FCMPE_0h as _, &[rn.as_operand()])
    }
    fn fccmps(
        &mut self,
        rn: impl OperandCast,
    ...
    fn dup8bw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP8bw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup4hw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP4hw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup2sw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP2sw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup16bw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP16bw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup8hw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP8hw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup4sw(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP4sw as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn dup2dx(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::DUP2dx as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn insb(
        &mut self,
        rd: impl OperandCast,
    ...
    fn cmgtd_zero(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CMGTd_zero as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cmeqd_zero(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CMEQd_zero as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cmltd_zero(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CMLTd_zero as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cmged_zero(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CMGEd_zero as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cmled_zero(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CMLEd_zero as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn suqaddb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADDb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabsb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABSb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqaddh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADDh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadds(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADDs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqaddd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADDd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn absd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqaddb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADDb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqnegb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEGb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqaddh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADDh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqnegh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEGh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadds(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADDs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqnegs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEGs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqaddd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADDd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqnegd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEGd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn negd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEGd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn suqadd2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SUQADD2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqabs2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQABS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn abs2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ABS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn usqadd2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::USQADD2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqneg2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQNEG2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn neg2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NEG2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp1d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP1d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp1d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP1d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp1d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP1d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp1d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP1d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlp2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLP2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sadalp2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADALP2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlp2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLP2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uadalp2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADALP2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cls4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn clz4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CLZ4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cnt8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CNT8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn not8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NOT8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rbit8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::RBIT8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn cnt16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::CNT16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn not16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::NOT16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rbit16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::RBIT16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn mvn8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MVN8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn mvn16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MVN16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev16_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV16_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev16_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV16_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev64_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV64_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn rev32_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::REV32_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtnb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTNb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtnh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTNh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtns(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTNs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtunb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUNb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtnb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTNb as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtunh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUNh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtnh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTNh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtuns(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUNs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtns(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTNs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtun_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUN_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtn_8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN_8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtun_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtn_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtun_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtn_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn2_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN2_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn2_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN2_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn xtn2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::XTN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtn2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn uqxtn2_16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN2_16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtun2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtn2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqxtun2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SQXTUN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uqxtn2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UQXTN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fabs2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fneg2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEG2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrt2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRT2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fabs4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fabs2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fneg4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEG4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrt4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRT4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fneg2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEG2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrt2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRT2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn frecpes(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPEs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpxs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPXs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecped(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPEd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpxd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPXd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrtes(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTEs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrted(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTEd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn urecpe2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::URECPE2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpe2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPE2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ursqrte2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::URSQRTE2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrte2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTE2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn urecpe4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::URECPE4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpe4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPE4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpe2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPE2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ursqrte4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::URSQRTE4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrte4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTE4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrte2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTE2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fcvtnsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtash(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFh as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fcvtpsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpeh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPEh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpxh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPXh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnuh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmuh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtauh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFh as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fcvtpuh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzuh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrteh(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTEh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintn4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTN4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintm4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTM4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtns4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtms4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtas4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtf4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTF4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fabs4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintp4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTP4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintz4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZ4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtps4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzs4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZS4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpe4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPE4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinta4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTA4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintx4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTX4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnu4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNU4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmu4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMU4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtau4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAU4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtf4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTF4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fneg4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEG4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinti4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTI4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpu4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPU4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzu4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZU4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrte4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTE4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrt4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRT4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintn8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTN8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintm8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTM8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtns8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtms8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtas8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtf8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTF8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fabs8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FABS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintp8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTP8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintz8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZ8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtps8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzs8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZS8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frecpe8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRECPE8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinta8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTA8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintx8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTX8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnu8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNU8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmu8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMU8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtau8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAU8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtf8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTF8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fneg8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FNEG8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinti8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTI8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpu8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPU8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzu8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZU8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frsqrte8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRSQRTE8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fsqrt8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FSQRT8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn mov8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MOV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn mov16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::MOV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn sxtl_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtl_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtl_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtl2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtl2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sxtl2_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SXTL2_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uxtl2_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UXTL2_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn shll_8h_8(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHLL_8h_8 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn shll_4s_16(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHLL_4s_16 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn shll_2d_32(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHLL_2d_32 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn shll2_8h_8(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHLL2_8h_8 as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn fcvtnss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtass(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtasd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTASd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzss(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzsd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZSd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnus(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmus(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtaus(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnud(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNUd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmud(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMUd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtaud(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAUd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpus(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzus(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpud(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPUd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzud(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZUd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtns2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtms2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtas2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtps2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzs2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZS2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnu2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNU2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmu2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMU2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtau2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAU2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpu2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPU2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzu2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZU2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtns4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtms4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtas4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtns2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtms2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtas2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtps4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzs4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZS4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtps2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzs2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZS2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnu4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNU4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmu4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMU4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtau4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAU4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtnu2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTNU2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtmu2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTMU2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtau2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTAU2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpu4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPU4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzu4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZU4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtpu2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTPU2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtzu2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTZU2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtl_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTL_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtl_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTL_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtl2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTL2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtl2_2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTL2_2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtfd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTFd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfs(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtfd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTFd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtf2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTF2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtf2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTF2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtf4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTF4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn scvtf2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SCVTF2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtf4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTF4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn ucvtf2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UCVTF2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtxns(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTXNs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtn_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtn_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn bfcvtn_4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::BFCVTN_4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtxn_2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTXN_2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtn2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtn2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn bfcvtn2_8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::BFCVTN2_8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fcvtxn2_4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FCVTXN2_4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintn2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTN2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintm2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTM2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintp2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTP2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintz2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZ2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinta2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTA2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintx2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTX2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinti2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTI2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintn4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTN4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintm4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTM4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintn2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTN2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintm2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTM2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintp4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTP4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintz4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZ4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintp2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTP2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintz2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTZ2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinta4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTA4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintx4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTX4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinta2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTA2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frintx2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTX2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinti4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTI4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frinti2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINTI2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32z2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Z2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64z2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Z2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32x2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32X2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64x2s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64X2s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32z4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Z4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64z4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Z4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32z2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32Z2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64z2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64Z2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32x4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32X4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64x4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64X4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint32x2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT32X2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn frint64x2d(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FRINT64X2d as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn smaxv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMAXV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sminv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMINV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn smaxv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMAXV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sminv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMINV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn umaxv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMAXV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uminv8b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMINV8b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn umaxv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMAXV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uminv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMINV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn smaxv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMAXV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sminv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMINV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn smaxv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMAXV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sminv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMINV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn saddlv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SADDLV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn smaxv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMAXV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sminv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SMINV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn addv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ADDV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn umaxv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMAXV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uminv16b(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMINV16b as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn umaxv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMAXV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uminv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMINV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uaddlv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UADDLV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn umaxv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMAXV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn uminv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::UMINV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn faddph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FADDPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminph(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINPh as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn faddps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FADDPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn faddpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FADDPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminps(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINPs as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminpd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINPd as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminv4h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINV4h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminv8h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINV8h as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxnmv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXNMV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fmaxv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMAXV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminnmv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINNMV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn fminv4s(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::FMINV4s as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sqdmulhh_elem(
        &mut self,
        rd: impl OperandCast,
    ...
    fn movid(&mut self, rd: impl OperandCast, imm64: impl OperandCast) {
        self.emit_n(Opcode::MOVId as _, &[rd.as_operand(), imm64.as_operand()])
    }
    fn movi2d(&mut self, rd: impl OperandCast, imm64: impl OperandCast) {
        self.emit_n(Opcode::MOVI2d as _, &[rd.as_operand(), imm64.as_operand()])
    }
    ...
    fn fmov2si(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOV2si as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmov4hi(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOV4hi as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmov4si(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOV4si as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmov8hi(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOV8hi as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn fmov2di(&mut self, rd: impl OperandCast, imm: impl OperandCast) {
        self.emit_n(Opcode::FMOV2di as _, &[rd.as_operand(), imm.as_operand()])
    }
    fn pacia(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::PACIA as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn pacib(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::PACIB as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn pacda(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::PACDA as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn pacdb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::PACDB as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn autia(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AUTIA as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn autib(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AUTIB as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn autda(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AUTDA as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn autdb(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AUTDB as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn paciza(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::PACIZA as _, &[rd.as_operand()])
    }
    fn pacizb(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::PACIZB as _, &[rd.as_operand()])
    }
    fn pacdza(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::PACDZA as _, &[rd.as_operand()])
    }
    fn pacdzb(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::PACDZB as _, &[rd.as_operand()])
    }
    fn autiza(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::AUTIZA as _, &[rd.as_operand()])
    }
    fn autizb(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::AUTIZB as _, &[rd.as_operand()])
    }
    fn autdza(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::AUTDZA as _, &[rd.as_operand()])
    }
    fn autdzb(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::AUTDZB as _, &[rd.as_operand()])
    }
    ...
    fn xpaci(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::XPACI as _, &[rd.as_operand()])
    }
    fn xpacd(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::XPACD as _, &[rd.as_operand()])
    }
    ...
    fn ldaprb(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAPRB as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaprh(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAPRH as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaprw(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAPRw as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldaprx(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDAPRx as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn cmpp(&mut self, rn: impl OperandCast, rm: impl OperandCast) {
        self.emit_n(Opcode::CMPP as _, &[rn.as_operand(), rm.as_operand()])
    }
    ...
    fn stzgm(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STZGM as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn stgm(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::STGM as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ldgm(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LDGM as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn cfinv(&mut self) {
        self.emit_n(Opcode::CFINV as _, &[])
    }
    fn xaflag(&mut self) {
        self.emit_n(Opcode::XAFLAG as _, &[])
    }
    fn axflag(&mut self) {
        self.emit_n(Opcode::AXFLAG as _, &[])
    }
    ...
    fn setf8(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::SETF8 as _, &[rn.as_operand()])
    }
    fn setf16(&mut self, rn: impl OperandCast) {
        self.emit_n(Opcode::SETF16 as _, &[rn.as_operand()])
    }
    fn sb(&mut self) {
        self.emit_n(Opcode::SB as _, &[])
    }
    fn tcancel(&mut self, imm16: impl OperandCast) {
        self.emit_n(Opcode::TCANCEL as _, &[imm16.as_operand()])
    }
    fn tcommit(&mut self) {
        self.emit_n(Opcode::TCOMMIT as _, &[])
    }
    fn tstart(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TSTART as _, &[rt.as_operand()])
    }
    fn ttest(&mut self, rt: impl OperandCast) {
        self.emit_n(Opcode::TTEST as _, &[rt.as_operand()])
    }
    fn wfet(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::WFET as _, &[rd.as_operand()])
    }
    fn wfit(&mut self, rd: impl OperandCast) {
        self.emit_n(Opcode::WFIT as _, &[rd.as_operand()])
    }
    fn st64b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::ST64B as _, &[rt.as_operand(), rn.as_operand()])
    }
    fn ld64b(&mut self, rt: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::LD64B as _, &[rt.as_operand(), rn.as_operand()])
    }
    ...
    fn aese(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AESE as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn aesd(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AESD as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn aesmc(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AESMC as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn aesimc(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::AESIMC as _, &[rd.as_operand(), rn.as_operand()])
    }
    ...
    fn sha1h(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHA1H as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sha1su1(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHA1SU1 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sha256su0(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHA256SU0 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sm3tt1a(
        &mut self,
        rd: impl OperandCast,
    ...
    fn sha512su0(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SHA512SU0 as _, &[rd.as_operand(), rn.as_operand()])
    }
    fn sm4e(&mut self, rd: impl OperandCast, rn: impl OperandCast) {
        self.emit_n(Opcode::SM4E as _, &[rd.as_operand(), rn.as_operand()])
    }
