...
impl<'a> SseExtrqEmitter<Xmm, Imm> for Assembler<'a> {
    fn sse_extrq(&mut self, op0: Xmm, op1: Imm) {
        self.emit(SSE_EXTRQRI, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}

impl<'a> SseExtrqEmitter<Xmm, Xmm> for Assembler<'a> {
    fn sse_extrq(&mut self, op0: Xmm, op1: Xmm) {
        self.emit(SSE_EXTRQRR, op0.as_operand(), op1.as_operand(), &NOREG, &NOREG);
    }
}
...
impl<'a> Assembler<'a> {
    ...
    where Assembler<'a>: SseExtrqEmitter<A, B> {
    ...
    where Assembler<'a>: SseInsertqEmitter_2<A, B> {
    ...
    where Assembler<'a>: SseInsertqEmitter_3<A, B, C> {
